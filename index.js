...
// Regras:
// 1. Tokens completos (>2 letras) precisam bater por igualdade ou inclusao
// 2. Abreviacoes (1-2 letras) so sao aceitas se pelo menos 2 tokens completos ja bateram
// 3. Para nomes curtos (poucos tokens completos), exige minimo de 2 tokens completos batendo
//    ou todos os completos se houver menos de 2
...
// Conta matches de tokens completos
...
// Minimo de tokens completos exigidos:
// - Se PDF tem 3+ tokens completos: exige 2
// - Se PDF tem 2 tokens completos: exige 2 (todos)
// - Se PDF tem 1 token completo: exige 1 (todos)
// - Se PDF tem 0 tokens completos: rejeita (nao da pra identificar sem nenhum token completo)
...
// Com minimo de completos batendo, valida abreviacoes
var matchesAbrev = 0;
...
// Score geral: precisa de 60% dos tokens do PDF batendo
...
// CRUZAMENTO COM CONTAGEM E ABREVIACOES
async function cruzarEDarBaixa(pendentes, nomesPDF) {
const repassados = [];
const ausentes   = [];

// Conta quantas vezes cada nome aparece no PDF (agrupando por similaridade)
const contagemPDF = {};
for (var i = 0; i < nomesPDF.length; i++) {
var nomePDF = nomesPDF[i];
var chaveExistente = null;
var chaves = Object.keys(contagemPDF);
for (var j = 0; j < chaves.length; j++) {
if (nomesBatem(chaves[j], nomePDF) || nomesBatem(nomePDF, chaves[j])) {
...
// Controla baixas dadas por nome
const baixasDadas = {};
...
```
// Encontra correspondencia no PDF
...
var totalNoPDF   = contagemPDF[nomePDFCorr];
