...
function nomesBatem(nomePlanilha, nomePDF) {
  ...
  return (totalMatch / tPDF.length) >= 0.6;
}
...
// Extrai pacientes do PDF com nome e data de cada um
...
// CRUZAMENTO COM DATA — so da baixa se data do PDF for igual ou anterior ao registro
async function cruzarEDarBaixa(pendentes, pacientesPDF) {
  const repassados = [];
  const ausentes   = [];

  // Agrupa pacientes do PDF por nome (contando ocorrencias)
  const contagemPDF = {};
  ...
  for (var k = 0; k < ordenados.length; k++) {
    ...
    // Verifica se a data do registro NAO e futura em relacao as datas do PDF
    var dataRegistro = parseData(a.data);
    var datasUsaveis = contagemPDF[nomePDFCorr].ocorrencias;
    var dataPDFCorr  = datasUsaveis[jaFoiBaixado] ? parseData(datasUsaveis[jaFoiBaixado]) : null;

    // Se conseguiu ler a data do PDF, garante que o registro nao e de mes futuro
    if (dataRegistro && dataPDFCorr) {
      ...
      if (mesRegistro > mesPDF) {
        // Registro e de mes posterior ao PDF — nao da baixa
