...
function nomesBatem(nomePlanilha, nomePDF) {
  ...
  console.log("MATCH [" + nomePDF + "] vs [" + nomePlanilha + "] score=" + score.toFixed(2) + " completos=" + matchesCompletos + "/" + minimoCompletos);
  return score >= 0.6;
}
...
async function extrairPacientesPDF(pdfBase64) {
  try {
    ...
    console.log("PDF extraido:", texto.substring(0, 500));
    const parsed = JSON.parse(texto);
    const pacientes = parsed.pacientes || [];
    console.log("Total pacientes PDF:", pacientes.length);
...
async function cruzarEDarBaixa(pendentes, pacientesPDF) {
  ...
  console.log("Pendentes na planilha:", pendentes.length);
  for (var x = 0; x < pendentes.length; x++) {
    console.log("Pendente[" + x + "]:", pendentes[x].nome, pendentes[x].data);
  }

  // Agrupa pacientes do PDF por nome
  const contagemPDF = {};
  ...
  console.log("Agrupamento PDF:", JSON.stringify(Object.keys(contagemPDF).map(function(k) {
    return k + " x" + contagemPDF[k].ocorrencias.length;
  })));
  ...
  for (var k = 0; k < ordenados.length; k++) {
    ...
    console.log("Buscando [" + a.nome + "] -> correspondencia: " + (nomePDFCorr || "NENHUMA"));
    ...
    if (dataRegistro && dataPDFCorr) {
      ...
      console.log("Data registro: " + a.data + " mesRegistro=" + mesRegistro + " mesPDF=" + mesPDF);
      if (mesRegistro > mesPDF) {
        console.log("BLOQUEADO: registro de mes futuro");
